...
const InternRegistration = ({ setSelectedView }) => {
  ...
  const validateMobileNumber = async (value) => {
    ...
    try {
        ...
        console.log("Mobile number validation response:", response.data);

        // Check the "exists" field in the response
        return !response.data.exists; // Validation passes if "exists" is false
    } catch (error) {
        console.error("Mobile number validation error:", error);
        return false; // Fail validation if an error occurs
    }
};
  ...
  const validationSchema = Yup.object({
    ...
    mobileno: Yup.string()
      ...
      .test("check-mobile", "Mobile number already exists", validateMobileNumber),
    ...
    profile_image: Yup.mixed()
      ...
      .test("fileType", "Only JPG, PNG, JPEG files are allowed", (value) => {
        return (
          value &&
...
const handleImageChange = (e, setFieldValue) => {
  ...
  if (file) {
    ...
    // ⭐ Most important: update Formik's field
  ...
  try {
    ...
    Object.keys(registrationDetails).forEach((key) => {
      if (key === "profile_image") {
        formData.append("profile_image", registrationDetails.profile_image);
      } else {
        formData.append(key, registrationDetails[key]);
      }
    });
...
<div className="d-flex justify-content-between align-items-start" style={{ gap: '20px' }}>
  ...
  <div>
    <input type="file" accept="image/*" id="photo-upload" name="profile_image" onChange={(e) => handleImageChange(e, setFieldValue)} style={{ display: 'none' }} />
      ...
      >
          {selectedImage ? (
            ...
            <div style={{alignItems:"center", justifyContent:"center"}}>
              <Camera size={40} color="#ffffff" style={{marginLeft:"27%"}}/>
    ...
    <ErrorMessage style={{fontSize:"12px"}} name="profile_image" component="div" className="error-message" />

  </div>
</div>


            <div className="intern_reg_form_group" style={{marginTop:"20px"}}>
              <Field
                as={TextField}
                label="Mobile No"
                variant="outlined"
            ...
            <div className="intern_reg_form_group">
              ...
              <FormControl fullWidth required variant="outlined">
                ...
                >
                  ...
                  <MenuItem value="GenerativeAI">GenAI</MenuItem>
