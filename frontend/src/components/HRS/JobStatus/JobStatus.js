...
const JobStatus = ({ statusInfo }) => {
  ...
  const memoColumns = useMemo(() => [
    ...
    // { Header: 'Job ID', accessor: 'jobId' },
    {
      Header: 'Job ID', accessor: 'jobId',
      Cell: ({ row }) => (
        <Link style={{ textDecoration: 'none', color: 'blue', fontWeight: '600' }} to={`/hr_dash/job_desc/${row.original.jobId}`}>
          {row.original.jobId}
        </Link>
      )
    ...
    },
    {
      ...
      accessor: 'DriveDate',
      Cell: ({ row }) => {
        const date = row.original.DriveDate;
        const column = date ? date : 'Add Date';

        return (
          <p
            style={{ cursor: 'pointer', textDecoration: date ? 'none' : 'underline', color: '#007bff' }}
            onClick={() => openDateModal(row.original)}
          >
            {column}
          </p>
        );
  ...
  const saveDriveDate = async () => {
    try {
      const response = await apiService.put('/api/Drive-date', {
        jobId: selectedJobId,
        date: driveDate,
      });
        ...
        setData(prevData =>
          prevData.map(item =>
            item.jobId === selectedJobId
              ? { ...item, DriveDate: driveDate }
              : item
          )
        );
        toast.success("Drive Date Saved Successfully.")
    ...
    <>
      ...
      <Modal show={showDriveDateModal} onHide={() => setShowDriveDateModal(false)}>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={saveDriveDate}>Save</Button>
        </Modal.Footer>
      </Modal>
