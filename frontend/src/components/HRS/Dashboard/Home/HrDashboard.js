...
const HrPortal = () => {
  ...
  const [statistics, setStatistics] = useState([
    ...
    { title: 'Total Students Delayed', value: 0, link: '/hr_dash/delayed', color: '#459746ff', element: IoPersonAddSharp },
  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      try {
        ...
        const delayedResponse = await apiService.get(`/api/hr-statistics/?status=delayed&hrId=${HrId}`);
        ...
        setStatistics(prevStats => prevStats.map(stat => {
          ...
          switch (stat.title) {
            ...
            case 'Total Students Delayed':
              return { ...stat, value: delayedResponse.data.count };
