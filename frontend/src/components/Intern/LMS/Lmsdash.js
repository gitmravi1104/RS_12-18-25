...
const QuizModal = ({
  ...
  const renderQuizResults = () => {
    const incorrectQuestions = currentQuiz?.pages_data[0]?.question_list.filter(
      (question, index) => quizResponses[question.question_id] !== question.correct_answer
    );

    return (
      <DialogContent>
        <Typography variant="h6" color={'black'} className="Attempt_error">
          Your score: {quizScore !== null ? `${quizScore.toFixed(2)}%` : 'N/A'}
          {quizScore === 100
            ? ' - Great job! You can now proceed to the next content.'
            : ' - Please try again to unlock the next content.'}
        ...
        <Typography
          ...
          className="Attempt_detail"
          sx={{
            color: quizScore !== null && quizScore === 100 ? 'green' : 'red',
          }}
        >
          {quizScore !== null && correctAnswers / totalQuestions > 0.7
            ? "You're doing great! Keep it up!"
        ...
        {incorrectQuestions.length > 0 && (
          <div>
            ...
            {incorrectQuestions.map((question, index) => (
              <div key={question.question_id} className="Attempt_wrongAnswer" >
                ...
                <Typography style={{ paddingTop: "-10px", paddingRight: "10px" }}>
                  <strong>Your Answer:</strong> {quizResponses[question.question_id] || 'Not Answered'}
                </Typography>
...
const LMS_dash = () => {
  ...
  const isMaterialUnlocked = (course, topic, subTopic, materialIndex) => {
    ...
    const firstSubTopic = subTopics[0]; // Get the first subTopic name
    ...
    // Ensure the first material in the first subTopic of every topic is unlocked
    const isFirstMaterialInFirstSubTopic = subTopic === firstSubTopic && materialIndex === 0;

    if (Object.keys(courseStatus).length > 0) {
      return isCompleted || isFirstMaterialInFirstSubTopic || (materialIndex === 0 && courseStatus[course]?.topics?.[topic]?.subTopics?.[subTopic]?.status === true);
    }

    return isFirstMaterialInFirstSubTopic || materialIndex === 0;
  };
    ...
    return Object.entries(courses).map(([courseName, courseData]) => (
      <Grid container spacing={3} key={courseName} sx={{ mb: 4 }}>
        ...
        {Object.entries(courseData.topics).map(([topicName, topicData]) => {
          ...
          if (!courseStatus[courseName]) courseStatus[courseName] = { topics: {} };
          if (!courseStatus[courseName].topics[topicName]) {
            courseStatus[courseName].topics[topicName] = { status: true };
          }
          ...
          return (
            <Grid item xs={12} sm={6} md={4} key={topicName}>
              <TopicCard
                ...
                onViewLessons={() => handleViewLessons(courseName, topicName)}
                onTakeCourse={() => handleTakeCourse(courseName, topicName)}
                isLocked={false} // All topics are unlocked
              />
            </Grid>
          );
        })}
