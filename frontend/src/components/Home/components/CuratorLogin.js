...
const CuratorLogin = () => {
    ...
    const handleSubmit = async () => {
        if (formData.username && formData.password) {
            try {
                const response = await apiService.post('/api/Curatorlogin', {
                ...
                // Set new cookies
                Cookies.set('role', 'curator', { expires: expiresInThirtyDays });
                Cookies.set('verified', true, { expires: expiresInThirtyDays });
                Cookies.set('name', response.data.name, { expires: expiresInThirtyDays });
                Cookies.set('CID', response.data.CID, { expires: expiresInThirtyDays });


                navigate('/curator/allInterns');
            } catch (error) {
            ...
            <div className="container d-flex flex-column justify-content-center" style={{ height: '80vh' }}>
                <div className="border rounded shadow p-3 d-flex flex-column align-items-center bg-none" style={{ width: '100%', maxWidth: '500px' }}>
                    <h4 className='fw-bold mb-4 mt-2 text-nowrap text-white' style={{ fontFamily: 'monospace' }}>
                        Curator Login <i className="fa-solid fa-right-to-bracket"></i>
                    </h4>
