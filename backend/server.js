...
app.use("/uploadsProJectsPortfolio", express.static(path.join(__dirname, "../uploadsProJectsPortfolio")));
app.use('/uploads', express.static('uploads'));
app.use(bodyParser.json());
app.use(express.urlencoded({ extended: true }));
app.use(cors());
// Projects Portfoilio Backend 


const TableQuery = `
CREATE TABLE IF NOT EXISTS PROJECTS(
ID BIGINT AUTO_INCREMENT PRIMARY KEY,
PROJECT_TITILE VARCHAR(255) NOT NULL,
TEAM_SIZE BIGINT NOT NULL,
TEAM_MEMBERS TEXT NOT NULL,
PROJECT_DESCRIPTION TEXT NOT NULL,
PROJECT_URL VARCHAR(255) NOT NULL,
PROJECT_LOGO VARCHAR(255) NOT NULL,
PROJECT_DOCUMENT VARCHAR(255) NOT NULL,
CREATED_AT TIMESTAMP NOT NULL
);
`;
// Database queries
const insertQuery = "INSERT INTO PROJECTS (PROJECT_TITILE , TEAM_SIZE , TEAM_MEMBERS, PROJECT_DESCRIPTION, PROJECT_URL, CREATED_AT, PROJECT_LOGO,PROJECT_DOCUMENT ) VALUES(?,?,?,?,?,?,?,?)"
const getAllProjects = "SELECT * FROM PROJECTS";
const getProjectById = "SELECT * FROM PROJECTS WHERE ID = ?";
const deleteProectsById = "DELETE FROM PROJECTS WHERE ID = ?";
const checkProjectExist = "SELECT * FROM PROJECTS WHERE PROJECT_TITILE  = ?"
const updtaeQueryById = "UPDATE PROJECTS SET PROJECT_TITILE = ? , TEAM_SIZE = ?, TEAM_MEMBERS = ?, PROJECT_DESCRIPTION = ?, PROJECT_URL = ? WHERE ID = ?"
const updtaeQueryByIdForImage = "UPDATE PROJECTS SET PROJECT_LOGO = ? WHERE ID = ?"
const updtaeQueryByIdForDocument = "UPDATE PROJECTS SET PROJECT_DOCUMENT = ? WHERE ID = ?"

db.query(TableQuery, (err, res) => {
  if (err) {
    console.log(err);
    return console.log("Table was not created" + err);
  } else return console.log("Proects Portfolio Table was created successfully" + res);
});

// to check the api's working are not 
app.get('/projects/check', (req, res) => {
  return res.status(200).json("welcome to the Projects Portfolio");
})

// uploaded directory
const uploadDir = path.join(__dirname, `../uploadsProJectsPortfolio/`);
...
const storage1 = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    cb(null, `project_profile_${Date.now()}${ext}`);
  }
});
...
app.post('/projects/save', upload1.fields([{ name: 'IMAGE', maxCount: 1 }, { name: 'DOCUMENT', maxCount: 1 }]), (req, res) => {
  const { PROJECT_TITILE, TEAM_SIZE, TEAM_MEMBERS, PROJECT_DESCRIPTION, PROJECT_URL } = req.body;

  // Check if file exists
  if (!req.files || !req.files.IMAGE || !req.files.DOCUMENT) {
    return res.status(400).json({ message: "Image and Document are required." });
  }


  const PROJECT_LOGO = `/uploadsProJectsPortfolio/${req.files.IMAGE[0].filename}`;
  const PROJECT_DOCUMENT = `/uploadsProJectsPortfolio/${req.files.DOCUMENT[0].filename}`;

  // Validate required fields
  if (!PROJECT_TITILE || !TEAM_SIZE || !TEAM_MEMBERS || !PROJECT_DESCRIPTION || !PROJECT_URL) {
    return res.status(400).json({ message: "All fields are required." });
  }

  // Check for duplicate entries
  db.query(checkProjectExist, [PROJECT_TITILE], (err, result) => {
    if (err) return res.status(500).json(err);

    if (result.length > 0) {
      return res.status(400).json({ message: "PROJECT_TITILE already exists." });
    }

    // Insert into database
    db.query(insertQuery, [PROJECT_TITILE, TEAM_SIZE, TEAM_MEMBERS, PROJECT_DESCRIPTION, PROJECT_URL, istTime, PROJECT_LOGO, PROJECT_DOCUMENT], (err, result) => {
      if (err) return res.status(500).json({ error: "Insert failed", details: err });
      else {
        return res.status(201).json({ message: "Proect added successfully", userId: result.insertId });
      }
...
app.get('/projects/getAll', (req, res) => {
  db.query(getAllProjects, (err, result) => {
...
app.get('/projects/getById/:id', (req, res) => {
  const id = req.params.id;
  if (!id) return res.status(400).json({ message: " ID Parameter is required" })
  else {
    db.query(getProjectById, [id], (err, result) => {
      if (err) return res.status(400).json(err);
      else {
        if (result.length > 0) {
...
// to delete teh data basedd on the id parameter
app.delete('/projects/delete/:id', (req, res) => {
  ...
  db.query(getProjectById, [id], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      db.query(deleteProectsById, [id], (deleteErr, deleteResult) => {
        if (deleteErr) {
...
// to update the data based on the id parameter
app.put('/projects/update/:id', (req, res) => {
  ...
  db.query(getProjectById, [id], (err, result) => {
    ...
    if (result.length > 0) {
      // After checking, proceed to delete
      db.query(updtaeQueryById, [PROJECT_TITILE, TEAM_SIZE, TEAM_MEMBERS, PROJECT_DESCRIPTION, PROJECT_URL, id], (updateErr, updateResult) => {
        if (updateErr) {
...
app.put('/projects/update-image/:id', upload1.single("image"), (req, res) => {
  const id = req.params.id;
  if (!id) {
    return res.status(400).json({ message: "ID parameter required" });
  }
  ...
  const PROJECT_LOGO = `/uploadsProJectsPortfolio/${req.file.filename}`; // Path to store in DB
  ...
  db.query(getProjectById, [id], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      db.query(updtaeQueryByIdForImage, [PROJECT_LOGO, id], (updateErr, updateResult) => {
        if (updateErr) {
          return res.status(500).json({ error: "Image update failed", details: updateErr });
        }
        return res.status(200).json({ message: "Image successfully updated for ID: " + id, PROJECT_LOGO });
      });
    } else {
      return res.status(404).json({ message: "No record found for ID: " + id });
    }
...
// Route to update the image only
app.put('/projects/update-document/:id', upload1.single("document"), (req, res) => {
  const id = req.params.id;
  if (!id) {
    return res.status(400).json({ message: "ID parameter required" });
  }
  ...
  const PROJECT_DOCUMENT = `/uploadsProJectsPortfolio/${req.file.filename}`; // Path to store in DB
  ...
  db.query(getProjectById, [id], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      db.query(updtaeQueryByIdForDocument, [PROJECT_DOCUMENT, id], (updateErr, updateResult) => {
        if (updateErr) {
          return res.status(500).json({ error: "document update failed", details: updateErr });
        }
        return res.status(200).json({ message: "document successfully updated for ID: " + id, PROJECT_DOCUMENT });
      });
    } else {
      return res.status(404).json({ message: "No record found for ID: " + id });
    }
...
app.get('/projects/download/:filename', (req, res) => {
  const filename = req.params.filename;
  const filePath = path.join(__dirname, '../uploadsProJectsPortfolio', filename);
...
const { default: AccessManagementIntern } = require('../frontend/src/components/SuperAdmin/intern_data/AccessManagementIntern');
...
app.get('/api/applications/:jobId', async (req, res) => {
  ...
  const sql = `
    SELECT 
      a.*, 
      COALESCE(i.domain, g.domain) AS domain
    FROM applied_students a
    LEFT JOIN intern_data i
      ON a.candidateID = i.candidateID
    LEFT JOIN guest_data g
      ON a.candidateID = g.guestID
    WHERE a.jobId = ?
  `;
...
app.post(
  '/api/register/intern',
  uploadProfile.single('profile_image'),  // ⭐ ADD middleware
  async (req, res) => {
    console.log(req);
    ...
    try {
      ...
      // Validate if email exists in intern_data
      const existingIntern = await query(
        'SELECT email FROM intern_data WHERE email = ?',
        [email]
      );
      ...
      if (existingIntern.length > 0) {
        return res.status(400).json({
          ...
          suggestion: 'Please use a different email address or contact admin.'
        });
      }

      // Validate if email exists in intern_requests
      const existingRequest = await query(
        'SELECT email FROM intern_requests WHERE email = ?',
        [email]
      );
      ...
      const profileImage = req.file.filename; // saved by multer

      // INSERT NEW INTERN REQUEST
      const sql = `
        INSERT INTO intern_requests
        (fullName, email, mobileNo, altMobileNo, address, batchno, 
         modeOfInternship, belongedToVasaviFoundation, domain, profile_img)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
      ...
      return res.status(500).json({
        message: 'Server error while registering intern'
      });
    }
  }
);
...
app.post(
  '/api/register/guest',
  uploadProfile.single('profile_image'),   // ⭐ ADD IMAGE UPLOAD MIDDLEWARE
  async (req, res) => {
    try {
      const {
        fullName,
      ...
      // Check email in guest_data
      const data1 = await query(
        'SELECT email FROM guest_data WHERE email = ?',
        [email]
      );
      ...
      // Check email in guest_requests
      const data2 = await query(
        'SELECT email FROM guest_requests WHERE email = ?',
        [email]
      );
      ...
      // INSERT new guest request
      const sql = `
        INSERT INTO guest_requests
        (fullName, email, mobileno, altmobileno, address, batchno, 
         modeOfTraining, program, domain, megadriveStatus,
         BelongedToVasaviFoundation, profile_img)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
      ...
      return res.status(500).json({
        message: 'Server error while registering guest'
      });
    }
  }
);
...
app.post('/api/Curatorlogin', [
  ...
  try {
    const rows = await query('SELECT * FROM curator WHERE username = ? AND password = ?', [username, password]);
    if (rows.length > 0) {
      const user = rows[0];
      console.log(user.name, "Logged in successfully");
      res.status(200).json({ message: 'Logged in successfully', name: user.name, CID: user.CID });
    } else {
      res.status(400).json({ message: 'Invalid credentials' });
    }
  } catch (err) {
...
app.get("/api/intern-view-jobs/:id", async (req, res) => {
  ...
  try {
    ...
    const jobs = await query(`
      SELECT * FROM jobs 
      WHERE DATE(lastDate) > CURDATE()
      AND JSON_CONTAINS(domains, JSON_QUOTE(?), '$') 
      AND jobId NOT IN (
        SELECT jobID FROM applied_students WHERE candidateID = ?
      )
    `, [internDomain, candidateId]);
...
// *Newly added code for AccessManagementIntern* (Ends in 2257 line)
// GET candidate access
app.get("/api/candidate_access/:id", async (req, res) => {
  const candidateID = req.params.id;
  try {
    const rows = await query(
      "SELECT accessRights FROM intern_data WHERE candidateID = ?",
      [candidateID]
    );
    ...
    // If no access set yet, return empty array
    const access = rows[0].accessRights ? JSON.parse(rows[0].accessRights) : [];
...
// UPDATE candidate access
app.put("/api/candidate_access/:id", async (req, res) => {
  const candidateID = req.params.id;
  const { accessRights } = req.body; // comes from frontend

  if (!Array.isArray(accessRights)) {
    return res.status(400).json({ message: "Invalid accessRights format" });
  }

  try {
    await query(
      "UPDATE intern_data SET accessRights = ? WHERE candidateID = ?",
      [JSON.stringify(accessRights), candidateID]
    );
...
//END
// *Newly added code for AccessManagementIntern* (Ends in 2323 line)
// GET candidate access
// app.get("/api/candidate_access/:id", async (req, res) => {
//   const candidateID = req.params.id;
//   try {
...
//     if (rows.length === 0) {
//       // Candidate not found, return safe default
//       return res.status(200).json({ access: [] });
//     }

//     // Parse JSON, fallback to empty array if null
//     let access = [];
//     try {
//       access = rows[0].accessRights ? JSON.parse(rows[0].accessRights) : [];
//     } catch (parseErr) {
//       console.error("Error parsing accessRights:", parseErr);
//       access = [];
//     }
...
//   } catch (err) {
//     console.error("Error fetching access:", err);
//     res.status(500).json({ access: [], message: "Server error" });
//   }
// });
...
//     const rows = await query(
//       "SELECT candidateID FROM intern_data WHERE candidateID = ?",
//       [candidateID]
//     );
      ...
      if (!existingEmails.has(intern.email) && !existingPhones.has(intern.mobileNo)) {
        ...
        await query(
          `INSERT INTO intern_data 
            (candidateID, fullName, email, mobileNo, altMobileNo, domain, belongedToVasaviFoundation, address, batchNo, modeOfInternship, profile_img, dateAccepted, endDate) 
  ...
  try {
    ...
    for (const guest of guests) {
      if (!existingEmails.has(guest.email) && !existingPhones.has(guest.mobileno)) {
        ...
        await query(
          'INSERT INTO guest_data (guestID, fullName, email, mobileno, altmobileno, address, batchno, modeOfTraining, program, domain, megadriveStatus, BelongedToVasaviFoundation, dateAccepted, profile_img) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, NOW(), ?)',
          [
            newGuestID,
            guest.fullName,
            guest.email,
...
app.get('/api/generate-certificate-id/:role/:month', async (req, res) => {
  ...
  // Option B: sanitize and shorten
  const roleInitial = role.replace(/[^a-zA-Z]/g, '').substring(0, 3).toUpperCase();

  try {
    const sql = `SELECT certificationId 
                 FROM certificates 
                 WHERE certificationId LIKE ? 
    ...
    query(sql, [likePattern], (err, result) => {
      ...
      let newSequenceNumber = '01';

      if (result.length > 0) {
        const lastId = result[0].certificationId;
        const lastMonth = lastId.slice(5, 7);
        if (lastMonth === month) {
          let lastSequenceNumber = parseInt(lastId.slice(-2), 10);
          newSequenceNumber = (lastSequenceNumber + 1).toString().padStart(2, '0');
        }
...
app.get('/api/dashboard-achievements-status/:internID', async (req, res) => {
  ...
  try {
    const result = await query(`SELECT certificate FROM intern_data WHERE candidateID = ?`, [internID]);
    console.log(result);
    ...
    const isCertificateUnlocked = result[0].certificate === 1;

    // Example logic: 1 default achievement + 1 if certificate is unlocked
    const totalAchievements = 1 + (isCertificateUnlocked ? 1 : 0);
...
//       SELECT 
//         h.HRid AS hr_id,
//         h.fullName AS hr_name,
//         COUNT(DISTINCT j.jobId) AS jobs_posted,
//         SUM(j.status = 'profiles-sent') AS sent_profiles,
//         SUM(j.status IN ('drive-done','offer-received')) AS drives_done,
//         COUNT(DISTINCT j.jobId) - SUM(j.status IN ('drive-done','offer-received')) AS drives_pending
//       FROM hr_data h
//       LEFT JOIN jobs j 
//         ON j.postedBy = h.HRid
//         AND j.DriveDate BETWEEN ? AND ?
//       GROUP BY h.HRid, h.fullName
//       ORDER BY h.fullName;
//     `;
  ...
  try {
    ...
    const sqlQuery = `
      SELECT 
        ...
        /* Jobs posted based on postedOn */
        COUNT(
          DISTINCT CASE 
            WHEN j.postedOn BETWEEN ? AND ? 
            THEN j.jobId 
          END
        ) AS jobs_posted,

        /* Status-based metrics based on DriveDate */
        SUM(
          CASE 
            WHEN j.PostedOn BETWEEN ? AND ? 
                 AND j.status = 'profiles-sent'
            THEN 1 ELSE 0 
          END
        ) AS sent_profiles,
        ...
        SUM(
          CASE 
            WHEN j.DriveDate BETWEEN ? AND ? 
                 AND j.status IN ('drive-done','offer-received')
            THEN 1 ELSE 0 
          END
        ) AS drives_done,
      ...
      FROM hr_data h
      LEFT JOIN jobs j ON j.postedBy = h.HRid
      GROUP BY h.HRid, h.fullName
      ORDER BY h.fullName;
    `;

    const params = [
      startDate, endDate, // jobs_posted
      startDate, endDate, // sent_profiles
      startDate, endDate, // drives_done
      startDate, endDate, // drives_pending total
      startDate, endDate  // drives_pending completed
    ];
...
app.get('/api/domain-stats', async (req, res) => {
  ...
  const domainOptions = [
    "Android Developer", "Flutter", "UiUx", "Python Full Stack", "Java Full Stack", "Mern Full Stack",
    "Testing Tools", "Scrum Master", "Business Analyst", "Dot Net",
    "Data Science", "Cyber Security", "Cloud Data Engineer",
    "DevOps & Cloud Computing",
    "Project Management & Agile", "SalesForce",
...
app.get('/api/job-stats/:id', async (req, res) => {
  ...
  try {
    ...
    SELECT 
      ...
      SUM(CASE WHEN applied_students.status = 'qualified' THEN 1 ELSE 0 END) AS qualified,
      SUM(CASE WHEN applied_students.status = 'delayed' THEN 1 ELSE 0 END) AS Students_delayed,
      SUM(CASE WHEN applied_students.status = 'placed' THEN 1 ELSE 0 END) AS placed,
      SUM(CASE WHEN applied_students.status = 'not-placed' THEN 1 ELSE 0 END) AS not_placed,
      SUM(CASE WHEN applied_students.status = 'not-attended' THEN 1 ELSE 0 END) AS not_attended,
      SUM(CASE WHEN applied_students.status = 'not-interested' THEN 1 ELSE 0 END) AS not_interested,
      SUM(CASE WHEN applied_students.status = 'not-eligible' THEN 1 ELSE 0 END) AS not_eligible,
      SUM(CASE WHEN applied_students.status = 'eligible' THEN 1 ELSE 0 END) AS eligible,
      SUM(CASE WHEN applied_students.status = 'under-progress' THEN 1 ELSE 0 END) AS under_progress,
      SUM(CASE WHEN applied_students.status = 'level-1' THEN 1 ELSE 0 END) AS level_1,
      SUM(CASE WHEN applied_students.status = 'level-2' THEN 1 ELSE 0 END) AS level_2,
      SUM(CASE WHEN applied_students.status = 'level-3' THEN 1 ELSE 0 END) AS level_3     
    FROM
      applied_students 
    WHERE
...
app.post('/api/resume-data/:id', async (req, res) => {
  console.log(req.body);
  console.log(req.params);

  const id = req.params.id;
  const { personalData, academicData, professionalData, preview } = req.body;

  const Sqlquery = `
    INSERT INTO resumeData (internID, personalData, academicData, professionalData, photo)
    VALUES (?, ?, ?, ?, ?)
    ON DUPLICATE KEY UPDATE
      personalData = VALUES(personalData),
      academicData = VALUES(academicData),
      professionalData = VALUES(professionalData),
      photo = ?
  `;
  ...
  await query(Sqlquery, values, (err) => {
    if (err) {
      console.error('Error inserting/updating data:', err);
      res.status(500).json({ error: 'Failed to insert/update resume data.' });
    } else {
...
// GET API to fetch resume data by ID
app.get('/api/resume-data/:id', async (req, res) => {
  const id = req.params.id;
  console.log(id);
  const sqlQuery = `SELECT * FROM resumeData WHERE internID = ?`;

  await query(sqlQuery, [id], (err, results) => {
    if (err) {
      console.error('Error fetching data:', err);
...
app.put('/api/resume-data/:id', async (req, res) => {
  ...
  const sqlQuery = `
    UPDATE resumeData 
    SET 
      personalData = COALESCE(?, personalData), 
      academicData = COALESCE(?, academicData), 
      professionalData = COALESCE(?, professionalData), 
      photo = COALESCE(?, photo)
    WHERE internID = ?
  `;

  const values = [
    personalData ? JSON.stringify(personalData) : null,
    academicData ? JSON.stringify(academicData) : null,
    professionalData ? JSON.stringify(professionalData) : null,
    preview || null,
    id
  ];

  await query(sqlQuery, values, (err, result) => {
...
// GET API to fetch resume data by ID
app.get('/api/InternReport/:id', async (req, res) => {
  const internID = req.params.id;
  try {
    const rows = await query('SELECT * FROM intern_data WHERE candidateID = ?', [internID]);
    res.status(200).json(rows);
...
app.get("/api/SA_course-progress/:internID", async (req, res) => {
  const internID = req.params.internID;
  console.log(internID);

  try {
    // Step 1: Get intern's domain
    const [internData] = await query("SELECT domain FROM intern_data WHERE candidateID = ?", [internID]);
    ...
    if (courseData.length === 0) {
      return res.status(404).json({ message: "No course data found for the selected domain." });
    }

    // Step 3: Fetch intern's completed progress
    const courseStatusResult = await query("SELECT progress FROM course_status WHERE internID = ?", [internID]);

    let completedProgress = {};
    if (courseStatusResult.length > 0 && courseStatusResult[0].progress) {
      completedProgress = courseStatusResult[0].progress;
    }

    // Step 4: Organize course data
    const organizedData = {};
    courseData.forEach(item => {
      if (!organizedData[item.CourseName]) {
        organizedData[item.CourseName] = { topics: {} };
    ...
    // Step 5: Send full data without calculating percentages
    res.status(200).json({
      count: Object.keys(organizedData).length,
      completedCourses: Object.values(completedProgress).filter(course => course.status).length,
      organizedData,
...
app.get("/api/Placed_Students", async (req, res) => {
  console.log("called", req.query);

  try {
    const { year, month, hrId } = req.query;

    let startDate, endDate;

    if (!year) {
      // No year → everything till today
      startDate = '1900-01-01';
      endDate = new Date().toISOString().split('T')[0];
    }
    else if (year && month) {
    ...
    // Optional HR filter (enable if needed)
    const hrCondition = hrId ? `AND j.postedBy = '${hrId}'` : '';

    const dateCondition = `
      AND j.DriveDate BETWEEN '${startDate}' AND '${endDate}'
    `;

    const internStudents = await query(`
      SELECT 
        i.fullName, 
        i.profile_img, 
        i.domain,
        ...
        'intern' AS source
      FROM applied_students a
      INNER JOIN intern_data i ON a.candidateID = i.candidateID
    ...
    const guestStudents = await query(`
      SELECT 
        g.fullName, 
        g.profile_img, 
        g.domain,
        ...
        'guest' AS source
      FROM applied_students a
      INNER JOIN guest_data g ON a.candidateID = g.guestID
      INNER JOIN jobs j ON a.jobId = j.jobId
      WHERE a.status = 'placed'
      ${dateCondition}
      ${hrCondition}
    ...
    const allPlacedStudents = [...internStudents, ...guestStudents]
      .sort((a, b) => new Date(b.DriveDate) - new Date(a.DriveDate));

    res.status(200).json(allPlacedStudents);
...
app.put("/api/application-comment", async (req, res) => {
  ...
  try {
    const updateQuery = `
      UPDATE applied_students 
      SET comments = ? 
      WHERE applicationID = ?`;

    const result = await query(updateQuery, [comments, applicationID]);
    console.log(result);
...
//Super admin Extra jobs
app.get("/api/extra-jobs", async (req, res) => {
  console.log("called")
  try {
    const jobs = await query(`SELECT * from jobs where postedBy is NULL`);
    console.log(jobs)
    res.status(200).json(jobs);
  } catch (err) {
    console.log(err)
...
app.put("/api/assign-hr", async (req, res) => {
  const { jobId, postedBy } = req.body;
  console.log(req.body);

  if (!jobId || !postedBy) {
    return res.status(400).json({ message: "Job ID and HR ID are required" });
  }
  ...
  try {
    ...
    const result = await query(updateQuery, [postedBy, jobId]);
    console.log(result);

    if (result.affectedRows > 0) {
      res.status(200).json({ message: "HR assigned successfully" });
    } else {
...
//Store Drive Date
app.put("/api/Drive-date", async (req, res) => {
  ...
  try {
    ...
    const result = await query(updateQuery, [date, jobId]);
    console.log(result);
...
// Job Comment
app.put("/api/job-comment", async (req, res) => {
  ...
  try {
    ...
    const result = await query(updateQuery, [comments, jobId]);
    console.log(result);
...
app.get("/api/certificate-unlocked/:id", async (req, res) => {
  const internID = req.params.id;
  ...
  try {
    ...
    const isUnlocked = result[0]?.certificate === 1;

    res.status(200).json({ certificateUnlocked: isUnlocked });
  } catch (err) {
    console.error("Database query error: ", err);
    res.status(500).json({ message: "Server error" });
  }
...
app.post('/api/toggle_certificate_intern/:candidateID', async (req, res) => {
  const { candidateID } = req.params;
  const { certificate } = req.body;

  try {
    await query('UPDATE intern_data SET certificate = ? WHERE candidateID = ?', [certificate, candidateID]);
    res.status(200).json({ message: `Intern ${certificate ? "Enabled" : "Disabled"} successfully.` });
  } catch (error) {
...
app.put('/api/update_company/:companyID', async (req, res) => {
  ...
  try {
    ...
    await query(sqlQuery, [companyName, hrName, email, mobileNo, website, address, publishedHrID, companyID]);
...
app.delete('/api/delete_company/:companyID', async (req, res) => {
  const { companyID } = req.params;

  try {
    const sqlQuery = `DELETE FROM companies WHERE companyID = ?`;

    await query(sqlQuery, [companyID]);
...
app.delete('/api/delete_application', async (req, res) => {
  const { applicationID } = req.query; // get from query string

  try {
    const sqlQuery = `DELETE FROM applied_students WHERE applicationID = ?`;
    await query(sqlQuery, [applicationID]);
...
app.put("/api/handleChangeComapanyHR", async (req, res) => {
  const { companyID, hrId } = req.body;
  console.log(req.body);

  if (!companyID) {
    return res.status(400).json({ message: "Company data is missing!" });
  }
  ...
  try {
    ...
    const result = await query(updateQuery, [hrId, companyID]);
    console.log(result);
...
app.put('/api/intern_access/:id', async (req, res) => {
  const { id } = req.params;
...
app.get('/api/intern_access/:id', async (req, res) => {
  const { id } = req.params; // Get HR ID from the URL parameters
  console.log(req);

  console.log('Fetching access for Candidate:', id);
  ...
  try {
    ...
    const access = result.accessRights;
    res.json({ message: 'Access retrieved successfully', access });
  } catch (error) {
    console.error('Error retrieving access:', error);
    res.status(500).json({ message: 'Internal Server Error' });
...
app.get('/api/placement-stats', async (req, res) => {
  console.log("Placement API called");
  ...
  try {
    ...
    const sqlQuery = `
      SELECT 
        ...
        j.DriveDate AS drive_date,
        j.jobTitle AS role,
        j.domains AS domains,
        h.fullName AS placement_officer,
        j.comments AS comments,

        /* ================= APPLICATION STATS ================= */

        /* Total Applications */
        COUNT(a.candidateID) AS total_applications_received,

        /* Not Eligible */
        COUNT(
          CASE 
            WHEN a.status = 'not-eligible' 
            THEN a.candidateID 
          END
        ) AS not_eligible,

        /* Profiles Sent = Total - Not Eligible */
        COUNT(a.candidateID)
        - COUNT(
            CASE 
              WHEN a.status = 'not-eligible' 
        ...
        COUNT(
          CASE 
            WHEN a.status = 'not-attended' 
            THEN a.candidateID 
          END
        ) AS not_attended,

        /* Attended = Profiles Sent - Not Attended */
        (
          COUNT(a.candidateID)
          - COUNT(
              CASE 
        ...
        COUNT(
          CASE 
            WHEN a.status = 'not-placed' 
            THEN a.candidateID 
          END
        ) AS rejected,

        /* Pending = Attended - (Selected + Rejected) */
        (
          (
            COUNT(a.candidateID)
            - COUNT(
      ...
      FROM jobs j
      LEFT JOIN hr_data h 
        ON h.HRid = j.postedBy
      LEFT JOIN applied_students a 
        ON a.jobID = j.jobId

      /* ===== Monthly / Year Filter ===== */
      WHERE 
        j.DriveDate BETWEEN ? AND ?
...
app.get('/api/hr-placement-stats', async (req, res) => {
  console.log("Placement API called");
  ...
  try {
    ...
    console.log("Date Range:", startDate, endDate, "HR:", hrId);

    /* ===== Dynamic WHERE condition ===== */
    let whereClause = `
      WHERE j.DriveDate BETWEEN ? AND ?
    `;
